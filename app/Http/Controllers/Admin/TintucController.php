...
class TintucController extends Controller
{
    public function getNewsAdd()
    {
        ...
        $diadiem = Diadiem::select('id','tendiadiem')->get()->toArray();
        return view('admin.module.tintuc.add',['cate'=>$cate,'diadiem'=>$diadiem]); 
     ...
     public function postNewsAdd(TintucAddRequest $request)
    {
        ...
        $news->loaitin_id =$request->sltCate;
         $news->diadiem_id =$request->sltCate1;
        $news->created_at =new DateTime();
        $news->save();
        if ($request->btnSaveNew) {
            return redirect()->route('them-tin-tuc')->with(['flash_level'=>'success','flash_message'=>'Bạn đã thêm thành công']);
        } else {
            return redirect()->route('danh-sach-tin-tuc')->with(['flash_level'=>'success','flash_message'=>'Bạn đã thêm thành công']);
        }
    ...
    public function getNewsDel($id){
        ...
        $news->delete();
         return redirect()->route('danh-sach-tin-tuc')->with(['flash_level'=>'success','flash_message'=> 'Xóa thành công']);
    }
     ...
     public function postNewsEdit(TintucEditRequest $request,$id){
        ...
        $news->save();
         if ($request->btnSaveNew) {
            return redirect()->route('them-tin-tuc')->with(['flash_level'=>'success','flash_message'=>'Bạn đã sửa thành công']);
        } else {
            return redirect()->route('danh-sach-tin-tuc')->with(['flash_level'=>'success','flash_message'=>'Bạn đã sửa thành công']);
        }
    }
